 ...
 *************************************************************************************/
function registerAPNS() {
    appID = "21197-8783752766";
    kpnsURL = "https://kms-demo.messaging.qa-konycloud.com";
...
function KPNSregCallback(status, result) {
    if (status == 400) {
        ...
        ksid = tmp["ksid"];
        if (tmp["statusCode"] == 200) kony.print("Device subscribed to KPNS Server sucessfully..");
        else kony.print("Failed to subscribe to KPNS Server!!" + tmp["message"]);
    }
}
/*************************************************************************************
